...
from app.controllers.auth_controllers.auth_verfication import (
    verfiy_email,
    verfiy_password,
)
from app.controllers.auth_controllers.pwd_context import create_hash
from app.db.get_session import DBSESSION
from app.controllers.auth_controllers.jwt_token import (
...
def register_route(
    formdata: Annotated[OAuth2PasswordRequestForm, Depends()], session: DBSESSION
):
    is_email = verfiy_email(session=session, username=formdata.username)
    if is_email:
